...
final class CalendarService: ObservableObject {
    ...
    func createEvent(for session: PomodoroSession) async -> String? {
        ...
        event.title = session.effectiveCalendarTitle
        event.startDate = session.startDate
        event.endDate = session.endDate
        event.calendar = calendar
        event.notes = notesWithAttribution(session)
    ...
    func generateICS(for session: PomodoroSession) -> String {
        ...
        let escapedNotes = icsEscape(notesWithAttribution(session))
...
SUMMARY:\(icsEscape(session.effectiveCalendarTitle))
